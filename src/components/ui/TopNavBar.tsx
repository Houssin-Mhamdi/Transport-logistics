...
export default function TopNavBar({ activePage = "home" }: TopNavBarProps) {
  ...
  return (
    <nav className="fixed top-0 w-full z-[100] bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 shadow-sm transition-all duration-300">
        ...
        <div className="hidden lg:flex items-center space-x-10">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className={`font-['Inter'] tracking-tight text-sm uppercase font-bold transition-all duration-300 relative group ${
            ...
            >
              {link.name}
              <span className={`absolute -bottom-1.5 left-0 h-0.5 bg-primary dark:bg-blue-500 transition-all duration-300 ${
